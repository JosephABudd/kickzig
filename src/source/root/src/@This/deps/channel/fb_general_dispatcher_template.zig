...
const line_import_channel =
    \\const {0s} = @import("{0s}.zig").Group;
    \\
;

const line_start_dispatcher_struct =
    \\
...
const line_dispatcher_struct_member =
    \\    {0s}: ?*{0s} = null,
    \\
;

const line_dispatcher_struct_init_start =
    \\
...
const line_dispatcher_struct_init_end =
    \\
    \\        // Initialize the running.
    \\        self.running = true;
    \\        self.condition.signal();
    \\        var thread = try std.Thread.spawn(.{ .allocator = self.allocator }, GeneralDispatcher.run, .{self});
...
const line_dispatcher_struct_deinit_start =
    \\    pub fn deinit(self: *GeneralDispatcher) void {
    \\        self.stop();
    \\
;

const line_dispatcher_struct_deinit_channel =
    \\        if (self.{0s}) |member| {{
    \\            member.deinit();
    \\        }}
    \\
...
const line_dispatcher_struct_deinit_end =
    \\        self.allocator.destroy(self);
    \\    }
    \\
;

const line_dispatcher_struct_dispatch_thru_run_start =
    \\
...
const line_dispatcher_struct_run_channel =
