...
pub fn rebuildPanelsZig(allocator: std.mem.Allocator, package_dir: std.fs.Dir, screen_name: []const u8) !void {
    ...
    const current_panel_names: [][]const u8 = try _filenames_.allFrontendBookScreenPanelNames(allocator, screen_name);
    defer {
...
fn directory(screen_name: ?[]const u8) !std.fs.Dir {
    ...
    var book_folder: std.fs.Dir = try std.fs.openDirAbsolute(folders.root_src_frontend_screen_book.?, .{});
    if (screen_name == null) {
        return book_folder;
    }
