...
pub const Template = struct {
    ...
    pub fn content(self: *Template) ![]const u8 {
        ...
        for (self.menu_item_names) |name| {
            line = try fmt.allocPrint(self.allocator, line_menu_item_enum, .{name});
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }

        try lines.appendSlice(line_menu_item_enum_end_label_start);
        for (self.menu_item_names) |name| {
            line = try fmt.allocPrint(self.allocator, line_tab_label, .{name});
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }

        {
            line = try fmt.allocPrint(self.allocator, line_struct_start, .{self.screen_name});
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }
        {
            line = try fmt.allocPrint(self.allocator, line_fn_frame_first_menu_item, .{self.menu_item_names[0]});
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }
        if (self.menu_item_names.len == 1) {
            try lines.appendSlice(line_fn_frame_end_first_menu_item);
        } else {
            for (self.menu_item_names, 0..) |name, i| {
                if (i == 0) {
                    continue;
                }
                line = try fmt.allocPrint(self.allocator, line_fn_frame_add_additional_menu_item, .{name});
                defer self.allocator.free(line);
                try lines.appendSlice(line);
            }
        }
        try lines.appendSlice(line_fn_frame_close_menu_items);
        try lines.appendSlice(line_fn_frame_switch_start);
        // line_fn_frame_switch_local_prong
        // line_fn_frame_switch_separate_prong
        for (self.menu_item_names, 0..) |name, i| {
            if (self.menu_item_has_panel[i] == true) {
                line = try fmt.allocPrint(self.allocator, line_fn_frame_switch_local_prong, .{name});
            } else {
                line = try fmt.allocPrint(self.allocator, line_fn_frame_switch_separate_prong, .{name});
            }
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }

        try lines.appendSlice(line_struct_end_fn_init_start);

        {
            line = try fmt.allocPrint(self.allocator, line_fn_init_end, .{ self.screen_name, self.menu_item_names[0] });
            defer self.allocator.free(line);
            try lines.appendSlice(line);
        }
...
const line_menu_item_enum =
    \\    {0s},
    \\
;

const line_menu_item_enum_end_label_start =
    \\    none,
    \\};
    \\
    \\/// KICKZIG TODO:
...
const line_tab_label =
    \\    const {0s}_label: []const u8 = "{0s}";
    \\
;
...
const line_fn_frame_first_menu_item =
    \\
...
const line_fn_frame_end_first_menu_item =
    \\            }
    \\
    \\
;

const line_fn_frame_add_additional_menu_item =
...
const line_fn_frame_close_menu_items =
...
const line_fn_frame_switch_start =
    \\        {
    \\            // Display the selected menu item's content.
    \\            switch (self.selected_menu_item) {
    \\
;
const line_fn_frame_switch_local_prong =
    \\                .{0s} => {{
...
const line_fn_frame_switch_separate_prong =
    \\                .{0s} => {{
...
const line_struct_end_fn_init_start =
    \\                .none => {
    \\                    return;
    \\                },
    \\            }
