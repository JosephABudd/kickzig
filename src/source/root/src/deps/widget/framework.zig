...
pub fn create() !void {
    ...
    {
        var dir: std.fs.Dir = try std.fs.openDirAbsolute(folders.root_src_deps_widget.?, .{});
        defer dir.close();
    ...
    {
        var dir: std.fs.Dir = try std.fs.openDirAbsolute(folders.root_src_deps_widget_tabbar.?, .{});
        defer dir.close();
